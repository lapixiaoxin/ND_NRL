...
def main(filename):
	...
	epoch = 10
	glove = learn_embeddings(walks, filename, dataset, epoch=epoch)

	eval_f1 = eval_metric_glove.Evaluator()
	ave_f1, average_pre, average_rec, pf1 = eval_f1.compute_f1(glove, filename, dataset)
...
if __name__ == "__main__":
	...
	# F1_List = F1_List[::-1]
	# F1_pre_List.sort()
	# F1_pre_List = F1_pre_List[::-1]
	# F1_rec_List.sort()
	# F1_rec_List = F1_rec_List[::-1]
	#
	# print F1_List
	# print f1_name
	# print sorted(f1_name.items(), lambda x, y: cmp(x[1], y[1]))
	#
	# print "tot average pre:" + str(sum(F1_pre_List[0:10]) / 10)
	# print "tot average rec:" + str(sum(F1_rec_List[0:10]) / 10)
	# print "tot average f1:" + str(sum(F1_List[0:10]) / 10)
	# print "tot average pf1:" + str(sum(F1_pf1_List[0:10]) / 10.0)
	...
	dataframe.to_csv("res_citeseer/res_glove.csv")
