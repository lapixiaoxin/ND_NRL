...
class Evaluator():
    ...
    def compute_f1(glove, filename, dataset):
        ...
        #y_pred = AgglomerativeClustering(n_clusters = true_cluster_size,linkage = "average",
            # affinity = "cosine").fit_predict(D_matrix)
            # y_pred=XMeans(random_state=5).fit_predict(D_matrix)
        # y_pred = AffinityPropagation(damping=0.5).fit_predict(D_matrix)
        # y_pred=hdbscan.HDBSCAN(min_cluster_size=3).fit_predict(D_matrix)
        ...
        # pre, rec, f1 = pairwise_precision_recall_f1(y_pred, dataset.label_list)
        # mm = confusion_matrix(dataset.label_list, y_pred)
        # rrr = precision_score(dataset.label_list, y_pred, average='macro')
        # aaa = accuracy_score(dataset.label_list, y_pred)
        # ccc = recall_score(dataset.label_list, y_pred)
        # mis = mutual_info_score(dataset.label_list, list(y_pred))
        # ars = adjusted_rand_score(dataset.label_list, list(y_pred))
        # purity = purity_score(np.array(dataset.label_list), y_pred)
